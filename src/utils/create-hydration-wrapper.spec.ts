...
it('should handle error at server-side', async () => {
  ...
  const app = createApp(() => () => h(WrappedComp));
...
it('should handle error at client-side when hydrating', async () => {
  ...
  let resolve!: (
    arg0: () => VNode<RendererNode, RendererElement, { [key: string]: any }>
  ) => void;
  let reject!: (arg0: Error) => void;
  let hydrate!: () => void | Promise<void>;
...
it('should handle error at client-side without hydration', async () => {
  ...
  let resolve!: (arg0: ConcreteComponent) => void;
  let reject!: (arg0: Error) => void;
