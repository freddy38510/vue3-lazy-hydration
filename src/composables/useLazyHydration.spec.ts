...
interface Result {
  client: ReturnType<typeof useLazyHydration> | null;
  server: ReturnType<typeof useLazyHydration> | null;
}
  ...
  const { container } = await withSSRSetup((isClient) => {
    const LazyComp: Component = {
      ...
      setup(props) {
        ...
        return () => h('button', { onClick: spyClick }, props.foo as boolean);
      },
    };
...
it('should run onHydrated hook when component has been hydrated and its children async components resolved', async () => {
  const result: Result & { client: { resolveAsyncComp?: () => void } | null } =
    {
      client: null,
      server: null,
    };
...
it('should throw error when used outside of the setup method', () => {
  const handler = vi.fn();
  const err = new Error(
